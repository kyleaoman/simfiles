...
from Hdecompose.SpringelHernquist2003 import sf_neutral_frac as correct_neutral_frac
import numpy as np
...
extractors["Lbox"] = extractor(
    ...
    hpath="/Header",
    attr="BoxSize",
    convert=lambda vals, raw, path, fname, hpath: raw / vals.h,
    units=U.Mpc,
    unit_convert=U.kpc,
)
...
extractors["p_mass"] = extractor(
    ...
    hpath="/Header",
    attr="MassTable",
    convert=lambda vals, raw, path, fname, hpath: raw[1] * 1e10 / vals.h,
    units=U.solMass,
    unit_convert=None,
)
...
extractors["sgns"] = extractor(
    ...
    hpath="/Subhalo/SubhaloGrNr",
    attr=None,
    convert=lambda vals, raw, path, fname, hpath: np.arange(raw.size)
    - vals.firstsub[raw],
    units=U.dimensionless_unscaled,
    unit_convert=None,
)
...
def subval(s):
    return lambda vals, raw, path, fname, hpath: raw[:, int(T[s])]
...
extractors["cops"] = extractor(
    ...
    attr=None,
    convert=lambda vals, raw, path, fname, hpath: raw * vals.a / vals.h,
    units=U.Mpc,
    unit_convert=U.kpc,
)
...
extractors["vcents"] = extractor(
    ...
    attr=None,
    convert=lambda vals, raw, path, fname, hpath: raw * np.sqrt(vals.a),
    units=U.km * U.s**-1,
    unit_convert=None,
)
...
extractors["msubfind"] = extractor(
    ...
    attr=None,
    convert=lambda vals, raw, path, fname, hpath: raw * 1e10 / vals.h,
    units=U.solMass,
    unit_convert=None,
)
...
def subval(s):
    return lambda vals, raw, path, fname, hpath: raw[:, int(T[s])] * 1e10 / vals.h
...
extractors["m_dm"] = extractor(
    ...
    attr=None,
    convert=lambda vals, raw, path, fname, hpath: np.ones(raw.shape, dtype=float)
    * vals.p_mass.to(U.solMass)
    / U.solMass,
    units=U.solMass,
    unit_convert=None,
)
...
extractors["T_g"] = extractor(
    ...
    hpath="/PartType0/InternalEnergy",
    attr=None,
    convert=lambda vals, raw, path, fname, hpath: (5 / 3 - 1)
    * raw
    * 1e10
    * 4
    * C.m_p.to(U.g).value
...
def subval(ie):
    return lambda vals, raw, path, fname, hpath: raw[:, ie]
...
extractors["mHneutral_g"] = extractor(
    ...
    attr=None,
    convert=lambda vals, raw, path, fname, hpath: vals.m_g.to(U.solMass).value
    * vals.fH_g
...
extractors["hsm_g"] = extractor(
    keytype="particle_g",
    filetype="snapshot",
    dependencies=("m_g", "rho_g", "a", "h"),
    hpath="/PartType0/Masses",  # just not going to use raw
    attr=None,
    convert=lambda vals, raw, path, fname, hpath: 2.5
    * np.power(3 / 4 / np.pi * vals.m_g / vals.rho_g, 1 / 3).to_value(U.kpc)
    * 0.7223517244643763,  # magic number is find_fwhm(_CubicSplineKernel().kernel) from martini
    units=U.kpc,
    unit_convert=None,
)
